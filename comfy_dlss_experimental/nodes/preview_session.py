    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            ...
            inputs=[
                ...
                NRProfile.Input("profile_b", display_name="当前效果 B", tooltip="连接正在调节的 NR Look。video_b 输出此效果的预览视频。"),
                NRProfile.Input("profile_a", display_name="对照效果 A（可选）", optional=True,
                                tooltip="不接时 A 是同尺寸、同色彩处理的输入对照；接另一个 NR Look 可比较两套效果，并增加一次 NR 处理。这里不会自动冻结该 Look 的参数。"),
                ...
                io.Boolean.Input("process_to_end", default=False, optional=True, display_name="处理到视频末尾",
                                 tooltip="自动读取输入 VIDEO 时长，从预览起点处理到末尾，忽略手动区间长度；起点 0 = 完整视频。单帧按钮仍只输出游标帧。长视频逐帧落盘，会按帧数和可用磁盘检查容量。"),
            ],
