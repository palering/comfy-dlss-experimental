...
- `workflows/dlss_native_preview.json`: Load Video, Video Guides, Video Input
  Adapter, Runtime Configuration, NR Look, Preview Session.
- `workflows/dlss_native_process.json`: the same inputs, Process Video, native
...
Choose start time, duration and scale, then click **渲染当前帧** or **渲染片段** inside
...
| 参数 | 含义 |
| --- | --- |
| 预览起点（`start_time`） | 相对输入 VIDEO（含上游裁剪）的起点，单位秒。 |
| 处理到视频末尾（`process_to_end`） | 开启后自动读取剩余时长，忽略手动 duration。起点 0 为完整 VIDEO。单帧按钮仍只输出一帧。 |
| 手动区间长度（`duration`） | 到末尾开关关闭时，决定片段实际长度；非结束时间、处理批大小或预热时长。遇末尾缩短。 |
| 预览尺寸（`preview_scale`） | 宽高各自的百分比；50% 约为 1/4 像素。实际改变 NR 输入及输出，非界面缩放或超分。 |
| 排队模式（`preview_mode`） | 顶部运行按钮：`range` 片段 / `frame` 单帧。卡片渲染按钮覆盖本次模式，不改保存值。 |
| 游标偏移（`cursor_time`） | 单帧时间 = 起点 + 偏移。偏移小于区间长度；片段模式忽略它。 |
| 当前效果 B / 对照效果 A | B 接 NR Look；A 不接为适配后的输入，接另一 Look 可比较两个效果，不自动冻结其参数。 |
| 历史设置（`contract`） | 可接 History Settings；默认前置 0.5 秒和新实例预热 120 次，不计入输出时长。 |
...
resolution, and enable **处理到视频末尾** for the remaining VIDEO. Process also
...
**参数与保存说明**. Input display names and tooltips are Chinese, but socket names,
existing socket names, widget order and default values remain compatible. The
...
- **Video Guides**: DIS/zero motion, analysis resolution, scene-cut reset and
  consistency diagnostics. It configures input guidance, not the NR look.
- **NR Look**: NR enable, model intensity, numeric style/preset, local tone,
  local/skin structure, automatic mask, original/result mix, and advanced UI
  correction/worker profile. The [parameter guide](nr-look.md) separates measured
...
Process Video's **处理到视频末尾** toggle uses the actual remaining VIDEO;
...
Configuration's optional **按需启动并常驻** toggle reuses one verified worker/model
...
There is no hot DLL switching. See [details](preview-performance.md#opt-in-resident-worker).
...
- NR Look: 24 half-second parameter/default/bypass cases, a two-look preview,
  ...
  [response matrix](nr-look.md#measured-response).
...
The [input adapter update](video-input-adapter.md) adds inline media diagnostics,
