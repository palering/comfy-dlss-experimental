...
[the Comfy integration](comfy-video-nodes.md). A working NGX invocation is
not proof of perceptual quality or temporal stability.
The follow-up [real-video validation](video-nr-validation.md) now implements
bounded clip preparation, motion guides and matched video exports.

## What runs, and what we build

```text
Linux Python (media inputs / settings / supervision)
...
`sidecar/protocol/frame-stream-v1.md` renderer protocol.
...
evidence are recorded in [video-nr-validation.md](video-nr-validation.md).
...
[resident lifecycle and validation](preview-performance.md#opt-in-resident-worker).
...
follows [sidecar-cpp-quality.md](sidecar-cpp-quality.md). Portable protocol
