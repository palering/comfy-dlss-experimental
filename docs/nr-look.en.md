...
| `nr_preset` (advanced) | Model default (recommended) / Experimental preset A, B, C | Native IDs 0–3; unknown effect semantics are not presented as named quality modes. |
| `nr_style` | Natural / Default style, Natural, Cinematic | Native IDs 0–2 respectively. Community names do not guarantee the same appearance across DLLs. |
...
| `worker_profile` (advanced) | Current compatible profile (recommended) / Experimental profile A, Current compatible profile (recommended), Experimental profile B, C | Native IDs 0–3 respectively; distinct from NR preset. Keep the current compatible setting (ID 1). |
...
The card follows ComfyUI's language setting with readable dropdown captions, not raw
...
See [NR input reconstruction](nr-input-reconstruction.en.md) for the distinction
...
only the HTTP status. The original validation had 87 Python / 6 JavaScript tests;
run the current suite for present coverage.
