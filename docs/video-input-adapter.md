...
its **检查输入（不运行 NR）** button can queue only its ancestors. No Worker,
...
**统一为 sRGB（SDR）** defaults off, including omitted fields in old API workflows.
...
First frame and cuts still reset history. External flow, per-frame isolation,
depth, and arbitrary masks are not advertised as implemented.

## Cache and execution

Source content, selected range/scale, guide settings and color policy all
participate in the prepared-cache key (version 5). Changing NR intensity can
...
video. The existing 30-second and 2 GiB per-range limits still apply.
...
The preview footer shows **颜色/光流缓存已复用** on a cache hit. The disk cache
