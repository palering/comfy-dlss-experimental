...
[node integration](comfy-video-nodes.md). This is not a replacement preview
application or a bottom panel.

## Dependencies and scope

Linux tests use the Comfy environment's existing NumPy, OpenCV with DIS, PyAV,
Pillow, FFmpeg and ffprobe. No packages or toolchains were installed. Tested:
...
timestamps instead of silently reinterpreting them. Clips are limited to 30
seconds plus at most 5 seconds of pre-roll; estimated combined color/motion
spools must fit 2 GiB. This is not a streaming full-length-video implementation.
...
SDR code values. There is no extra linearization/gamma pass. Export converts
...
root from [direct-nr-relay.md](direct-nr-relay.md). Supply the active Xwayland
