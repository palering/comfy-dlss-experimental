...
The sidecar is a separate Windows process that owns D3D12 and NGX. The Python
...
`protocol/sidecar-v1.schema.json` and `protocol/frame-stream-v1.md`. The Python
...
The production renderer must call the official static SDK wrapper. The supplied
...
is not a safe direct linker for that archive. We will use a narrow supported
MSVC build boundary rather than implementing substitute CRT/SEH symbols.
