...
function attach(node) {
  ...
  const liveHeading = element("summary", "Worker 实时状态");
  const historyHeading = element("summary", "执行记录 · 历史快照");
  const refresh = element("button", "立即刷新"); refresh.type = "button";
  const release = element("button", "终止并释放当前 Worker"); release.type = "button"; release.disabled = true;
  release.title = "会终止该 Worker 对应的整个 DLSS 任务；不清除视频缓存，不影响其他应用。";
  const actions = element("div"); actions.className = "dlss-runtime-actions";
  actions.append(refresh, release);
  const updateStatus = element("p", "展开后自动刷新；收起或页面隐藏时暂停。");
  ...
  device.append(element("summary", "GPU 整卡参考 · 包括其他应用"), deviceText);
  ...
  async function update() {
    ...
    if ((!live.open && !history.open) || document.hidden) {
      updateStatus.textContent = "自动刷新已暂停 · 展开状态/记录且页面可见时恢复";
      return;
    }
    pending = true;
    let next = 5000;
    try {
      ...
      updateStatus.textContent = `自动刷新 · ${next / 1000} 秒/次 · 更新于 ${new Date().toLocaleTimeString()}`;
      ...
      release.textContent = target?.mode === "idle" ? "释放空闲 Worker" : "终止并释放当前 Worker";
      release.title = target?.mode === "idle" ? "释放这个空闲实例的 GPU 和进程资源；输入缓存保留。"
        : "会终止该 Worker 对应的整个 DLSS 任务；不清除视频缓存，不影响其他应用。";
      release.disabled = !target || releasePending;
      state.textContent = active.length
        ? `本预设 ${active.length} 项任务执行中 · NR 串行；下方是本任务进程组采样。`
        : residents.length ? "本预设保留了常驻 Worker，仍占用以下资源。" : "本预设无存活的常驻实例 · 下次实际渲染时按所选模式启动。";
      if (completed[0]?.worker_state === "cleanup_failed") state.textContent += " 上次清理失败，请检查执行记录；不能据此断言无残留进程。";
      state.textContent += " 常驻仅复用相同 NR 参数；改 Look/尺寸/运行库时重建。";
      current.textContent = [...residents.map(worker => residentLines(worker).join("\n")), ...active.map(record => workerLines(record).join("\n"))].join("\n\n") || "没有实例可释放。历史占用请展开下方执行记录。";
      const releasedRecord = records.find(record => record.execution_id === releasedId);
      if (releasedRecord?.worker_state === "released") releaseStatus.textContent = "该任务 Worker 已完成释放。";
      else if (releasedRecord?.worker_state === "cleanup_failed") releaseStatus.textContent = "释放失败，请检查该任务的错误记录。";
      if (data.resident?.last_release?.worker_id === releasedId) releaseStatus.textContent = "该常驻实例已释放。";
      deviceText.textContent = (data.gpu?.gpus || []).map(g => `${g.name}\n整卡利用率 ${number(g.utilization_percent, "%")}\n整卡显存 ${number(g.memory_used_mib)} / ${number(g.memory_total_mib)}`).join("\n\n") || "整卡 GPU 数据不可用";
      if (data.gpu?.sampled_at) deviceText.textContent += `\n整卡采样于 ${new Date(data.gpu.sampled_at * 1000).toLocaleTimeString()}`;
      ...
      if (!completed.length) list.textContent = "没有已完成的匹配记录；正在执行的任务只显示在实时状态中。";
    } catch (error) {
      target = null; release.disabled = true;
      updateStatus.textContent = `${error.message} · 下次自动重试；保留画面可能已过期`;
    } finally { pending = false; schedule(next); }
  }
  release.onclick = async () => {
    const selected = target;
    if (!selected || releasePending) return;
    if (selected.mode === "cancel" && !window.confirm("终止当前 DLSS 任务并释放其 Worker？本次未完成的输出将不可用；输入缓存和其他程序不受影响。")) return;
    ...
    try {
      ...
      if (response.ok && result.accepted) {
        ...
        releaseStatus.textContent = "已请求释放；等待清理，尚未确认完成。";
      } else if (response.status === 409) releaseStatus.textContent = "该任务已结束或没有活动 Worker，无需释放。";
      else throw new Error("释放请求失败 " + response.status);
  ...
  async function retireSelectedPreset() {
    try {
      const response = await api.fetchApi("/dlss-experimental/executions");
      if (!response.ok) throw new Error("读取常驻状态失败");
      const data = await response.json();
      const preset = node.widgets?.find(w => w.name === "runtime_preset")?.value;
      if (!preset) return;
      for (const worker of matchingRecords(data.resident?.workers || [], preset)) {
        ...
        if (!result.ok && result.status !== 409) throw new Error("释放请求失败 " + result.status);
        const releaseResult = await result.json();
        if (result.ok && releaseResult.accepted) {
          releasedId = worker.worker_id;
          releaseStatus.textContent = "已关闭常驻：空闲实例将释放，执行中的任务完成后释放。";
        } else releaseStatus.textContent = "该实例状态已变化；请检查实时状态，必要时手动释放。";
      }
    } catch (error) { releaseStatus.textContent = error.message; }
    void update();
  }
